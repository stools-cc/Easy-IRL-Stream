...
static void fire_low_quality_actions(struct irl_source_data *data)
{
	...
	blog(LOG_INFO, "[%s] Low quality detected (%lld kbps)", PLUGIN_NAME,
...
static void fire_quality_recovered_actions(struct irl_source_data *data)
{
	...
	blog(LOG_INFO, "[%s] Quality recovered (%lld kbps)", PLUGIN_NAME,
	     (long long)data->current_bitrate_kbps);
...
static void fire_disconnect_actions(struct irl_source_data *data)
{
	...
	blog(LOG_INFO, "[%s] Firing disconnect actions", PLUGIN_NAME);
...
static void fire_reconnect_actions(struct irl_source_data *data)
{
	...
	blog(LOG_INFO, "[%s] Firing reconnect actions", PLUGIN_NAME);
...
{
	blog(LOG_INFO, "[%s] Client connected", PLUGIN_NAME);
...
{
	blog(LOG_INFO, "[%s] Client disconnected", PLUGIN_NAME);
